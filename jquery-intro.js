...
function addHolidayMessage() {
    ...
    if (currentMonthIndex === 11) { // dec
      ...
      if (h2Tag) {
        ...
        h3Tag.textContent = "Happy Holidays!";
...
function updateMonthTips() {
    ...
    const h3Tag = document.getElementById('month'); 
    if (h3Tag) {
      h3Tag.textContent = `Tips for ${currentMonthName}`; 
    }
  }
...
function appendCurrentYearToCopyright() {
    const currentDate = new Date();
  ...
  appendCurrentYearToCopyright();
  
  function displayMonthlySpecial() {
    const specials = [ //copy + paste specials tags wont appear on site
  ...
  "<p>Christmas trees!</p><p>We have sizes from 3' to 15' and lots of varieties. Find the perfect fit for your family while they last!</p>"
    ...
    const currentDate = new Date();
    const currentMonthIndex = currentDate.getMonth(); 
    const currentSpecial = specials[currentMonthIndex]; 
  
    const specialsDiv = document.getElementById('specials'); 
    if (specialsDiv) {
      specialsDiv.innerHTML = currentSpecial; 
    }
  }
  
  displayMonthlySpecial();
  ...
  function setSeasonAttributes() {
    ...
    if (seasonDisplay) {
      seasonDisplay.textContent = `Current Season: ${season}`;
    }
  }
